...
function rgbfloat = ahd_debayer(arifloat)

    fl_min = min(min(arifloat));
    scaled_float = (arifloat-fl_min)/2.5;
    
    r = expand_matrix(scaled_float, 2, 2, 2, 2);
    g = r;
    b = r;
  ...
  % :SECTION: Interpolate Red + Blue
    ...
    r_h(idx) = ( r(usft(idx)) + r(dsft(idx)) )/2 + ( 2*g(idx) - g_h(usft(idx)) - g_h(dsft(idx)) )/2;
    r_v(idx) = ( r(usft(idx)) + r(dsft(idx)) )/2 + ( 2*g(idx) - g_v(usft(idx)) - g_v(dsft(idx)) )/2;
    b_h(idx) = ( b(lsft(idx)) + b(rsft(idx)) )/2 + ( 2*g(idx) - g_h(rsft(idx)) - g_h(lsft(idx)) )/2;
    b_v(idx) = ( b(lsft(idx)) + b(rsft(idx)) )/2 + ( 2*g(idx) - g_v(rsft(idx)) - g_v(lsft(idx)) )/2;
    
    idx = f;
    idx(3:2:end-2, 3:2:end-2) = true;
    r_h(idx) = ( r(lsft(idx)) + r(rsft(idx)) )/2 + ( 2*g(idx) - g_h(lsft(idx)) - g_h(rsft(idx)) )/2;
    r_v(idx) = ( r(lsft(idx)) + r(rsft(idx)) )/2 + ( 2*g(idx) - g_v(lsft(idx)) - g_v(rsft(idx)) )/2;
    b_h(idx) = ( b(usft(idx)) + b(dsft(idx)) )/2 + ( 2*g(idx) - g_h(usft(idx)) - g_h(dsft(idx)) )/2;
    b_v(idx) = ( b(usft(idx)) + b(dsft(idx)) )/2 + ( 2*g(idx) - g_v(usft(idx)) - g_v(dsft(idx)) )/2;
  ...
  % :SECTION: Output
    ...
    rgbfloat(:,:,1) = r(3:end-2,3:end-2);
    rgbfloat(:,:,2) = g(3:end-2,3:end-2);
    rgbfloat(:,:,3) = b(3:end-2,3:end-2);
